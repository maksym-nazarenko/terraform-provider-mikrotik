...
func subcommandTest(args []string) error {
	...
	fs := flag.NewFlagSet("terraform_test", flag.ExitOnError)

	fs.StringVar(&sourceFile, "srcFile", "", "`path` to file with MikroTik client struc definition")
	fs.StringVar(&structName, "structName", "", "name of a struct to find in srcFile")
	fs.StringVar(&outFile, "outFile", "-", "`path` to write generated code to. Use '-' for stdout")
	if err := fs.Parse(args); err != nil {
		return err
	}
	...
	if err := codegen.GenerateTest(s, &buf); err != nil {
		return err
	}
